...
const emptyDefaults: ClinicFormValues = {
  name: "",
  address: "",
  contact: "",
};
...
const ClinicTable = () => {
  ...
  const [activeClinicId, setActiveClinicId] = useState<number | string | null>(
    null,
  );
  ...
  const onSubmit = (data: ClinicFormValues) => {
    const action = mode === "add" ? addClinic.mutate : updateClinic.mutate;

    action(data as Clinic, {
      onSuccess: () => {
        closeAndReset();
        refetch();
      },
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <DialogFooter>
                <Button
                  type="submit"
                  disabled={addClinic.isPending || updateClinic.isPending}
                >
                  {mode === "add"
                    ? addClinic.isPending
                      ? "Saving..."
        ...
        description={
          selectedClinic ? (
            <>
              ...
              <strong>{selectedClinic.name}</strong>? This action cannot be
              undone.
            </>
          ) : null
        }
