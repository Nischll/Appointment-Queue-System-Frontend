...
const emptyDefaults: DoctorFormValues = {
  name: "",
  email: "",
  phone: "",
};
...
const DoctorTable = ({ departmentId }: Props) => {
  ...
  const departmentName =
    doctors.length > 0 ? doctors[0]?.department_name : null;
  ...
  const onSubmit = (values: DoctorFormValues) => {
    ...
    const action = mode === "add" ? addDoctor.mutate : updateDoctor.mutate;
  ...
  const filteredDoctors = doctors.filter(
    (doctor): doctor is Doctor & { id: number | string } => !!doctor.id,
  );
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-sm text-center max-w-sm mb-4">
              {clinicName && departmentName
                ? `No doctors in ${departmentName} department at ${clinicName} yet.`
